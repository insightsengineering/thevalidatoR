...
date: "`r Sys.time()`"
author: 
  - "`r paste0('Repo server: ',Sys.getenv('GITHUB_SERVER_URL'))`"
  ...
  - "`r paste0('gh-action ID: ',Sys.getenv('GITHUB_ACTION'))`"
  - "`r paste0('Executed at: ',format(Sys.time(), "%a %b %d %X %Y"))`"
params:
...
\fancyhead[CO,CE]{`r sprintf('%s (v%s)', (dcf <- read.dcf(file.path(params$pkg_dir, 'DESCRIPTION')))[,'Package'], dcf[,'Version'])`}
\fancyhead[LO,LE]{Report created from SHA: `r paste0('Commit SHA: ',Sys.getenv('GITHUB_SHA'))`}
\fancyfoot[CO,CE]{Report created: `r format(Sys.time(), "%a %b %d %X %Y")`}
...
URL: `r Sys.getenv("GITHUB_SERVER_URL")`

GITHUB_SERVER_URL
GITHUB_ACTION
GITHUB_REPOSITORY
GITHUB_SHA - triggered by
GITHUB_REF
...
```{r riskmetric, echo = FALSE, eval = FALSE}
params$pkg_dir %>%
  riskmetric::pkg_ref() %>%
