...
This report is fully automated, so is limited to assess whether unit tests and documentation are present and can execute without error. An assessment would be required that the tests and documentation are meaningful. Validation is system dependent, so this report aims to reflect a 'common' system report that can be shared between companies, while acknowledging that each company would want to maintain installation / validation reports against the exact systems they deploy.
...
covr:::to_report_data(covr_results)$file_stats %>%
  dplyr::mutate(File = gsub("<.*?>", "",File)) %>%
  dplyr::arrange(dplyr::desc(File)) %>%
  dplyr::select(Lines,Relevant,Missed,Coverage) %>%
  helper_tabulate(
    caption = "Coverage of each function in the package",
    col_widths = c("6cm","3cm")
  )
```
